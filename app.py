...
import uuid
import os
import time # To potentially measure time for debugging if needed

# --- Page Configuration (Set Title, Icon, Layout) ---
st.set_page_config(page_title="Car Insights", layout="wide", page_icon="⚡") # Changed icon
...
# --- Custom CSS (Keep the previous enhanced CSS) ---
# (Keeping the CSS block from the previous version for brevity - assume it's here)
st.markdown(f"""
<style>
    ...
    .main {{
        ...
        padding: 25px;
        border-radius: 10px; /* Slight rounding of main area */
    }}
    /* Header styling */
    .app-header {{
     ...
     .app-subheader {{
        ...
        text-align: center;
        margin-bottom: 30px;
        font-weight: 300; /* Lighter subheader */
    }}
    /* Section Headers */
    h2 {{ /* Targeting Streamlit's default subheader element */
        color: {COLOR_PALETTE['primary']};
        border-bottom: 3px solid {COLOR_PALETTE['accent']}; /* Accent underline */
        padding-bottom: 8px;
        margin-top: 30px;
        margin-bottom: 20px;
        font-weight: 700;
    }}
     h3 {{ /* Targeting Streamlit's default subheader element */
        color: {COLOR_PALETTE['secondary']};
        margin-top: 25px;
        margin-bottom: 15px;
        font-weight: 700;
    ...
    /* Button styling */
    .stButton>button {{
        background-color: {COLOR_PALETTE['accent']}; /* Accent color for buttons */
        color: white;
    ...
    .stButton>button:hover {{
        background-color: #e6730d; /* Darker accent on hover */
    ...
    .stNumberInput input {{ padding: 10px; }} /* Adjust padding */
    ...
    .success-message {{
        ...
        font-weight: 700;
        background-color: #e6f9ef; /* Light green background */
        padding: 20px;
        border-radius: 8px;
        border: 1px solid {COLOR_PALETTE['success']};
        border-left: 6px solid {COLOR_PALETTE['success']}; /* Accent border */
        text-align: center;
        margin-top: 20px;
    ...
    .stExpander {{
        ...
        border-radius: 10px;
        margin-bottom: 20px;
        background-color: {COLOR_PALETTE['card_bg']}; /* White background for expander */
	...
	.stTabs [data-baseweb="tab"] {{
		...
		border-radius: 8px 8px 0 0; /* Top rounding */
		gap: 5px;
		padding: 10px 20px;
        font-weight: 700;
        font-size: 1.15em;
	...
	.stTabs [aria-selected="true"] {{
        ...
        color: {COLOR_PALETTE['primary']}; /* Active tab text color */
        border-bottom: 4px solid {COLOR_PALETTE['primary']}; /* Active indicator */
	}}

    /* Card styling for similar cars and saved predictions */
    .car-card {{
        background-color: {COLOR_PALETTE['card_bg']};
        border-radius: 10px;
        padding: 20px;
        ...
        height: 95%; /* Try to make cards in a row same height */
        display: flex;
    ...
    .car-card h4 {{ /* Make/Model Header */
     ...
     .car-card .card-content {{ /* Div for main content */
        flex-grow: 1;
     }}
     .car-card p {{ /* Paragraphs within cards */
        font-size: 0.95em;
        color: {COLOR_PALETTE['subtle_text']};
        margin-bottom: 5px;
        line-height: 1.4;
     }}
     .car-card .stMetric {{ /* Styling for metrics in cards */
         background-color: #f8f9fa; /* Light grey background for metric */
         border-radius: 6px;
         padding: 10px 15px;
         border: 1px solid {COLOR_PALETTE['border']};
         margin-top: 10px; /* Space above metric */
     }}
      .car-card .stMetric label {{
       ...
       .car-card .stMetric p {{ /* Metric value */
          font-size: 1.3em;
          font-weight: 700;
          color: {COLOR_PALETTE['accent']}; /* Use accent color for price */
          margin-bottom: 0; /* Remove bottom margin */
          line-height: 1.2;
       }}
       .icon {{ /* Simple class for icons */
           font-size: 1.1em;
           margin-right: 5px;
...
@st.cache_data # Cache the data loading (remains same)
def load_data(filepath):
    if not os.path.exists(filepath): st.error(f"Error: Data file not found at `{filepath}`"); return None
    try:
        df = pd.read_csv(filepath)
        required_cols = ['make', 'model', 'condition', 'year', 'odometer', 'mmr', 'sellingprice', 'state']
        if not all(col in df.columns for col in required_cols): st.error(f"Dataset missing required columns: {required_cols}"); return None
        df.replace([np.inf, -np.inf], np.nan, inplace=True)
        if 'vehicle_age' not in df.columns: df['vehicle_age'] = datetime.now().year - df['year']
        if 'mileage_per_year' not in df.columns: df['mileage_per_year'] = np.where(df['vehicle_age'] > 0, df['odometer'] / df['vehicle_age'], 0)
        if 'price_per_mile' not in df.columns: df['price_per_mile'] = np.where(df['odometer'] > 0, df['mmr'] / df['odometer'], 0)
        ...
        for col in ['year', 'condition', 'odometer', 'mmr', 'sellingprice', 'vehicle_age', 'mileage_per_year', 'price_per_mile']:
             try: df[col] = pd.to_numeric(df[col]) # Ensure numeric types robustly
             except Exception as e: st.warning(f"Could not convert column '{col}' to numeric: {e}") ; df[col] = 0 # Default or handle differently
        df['year'] = df['year'].astype(int) # Ensure year is int after numeric conversion
        return df
    except Exception as e: st.error(f"Error loading data: {e}"); return None

@st.cache_resource # Cache the model loading (remains same)
def load_model(filepath):
    if not os.path.exists(filepath): st.error(f"Error: Model file not found at `{filepath}`"); return None
    try: return joblib.load(filepath)
    except Exception as e: st.error(f"Error loading model: {e}"); return None

@st.cache_resource # Cache the scaler loading (remains same)
def load_scaler(filepath):
    if not os.path.exists(filepath): st.error(f"Error: Scaler file not found at `{filepath}`"); return None
    try:
        with open(filepath, 'rb') as f: return pickle.load(f)
    except Exception as e: st.error(f"Error loading scaler: {e}"); return None

# --- *** NEW CACHED FUNCTION for Preprocessing Dataset *** ---
...
def get_preprocessed_data_for_similarity(_df_original, _label_encoders, _scaler,
    ...
    """
    Preprocesses the entire dataset for similarity calculation and caches it.
    Uses underscore prefixes for arguments to help Streamlit's caching recognize changes.
    """
    ...
    for feature in categorical_features:
        if feature in _label_encoders:
            # Use .get() on classes_ attribute for safety
            ...
            df_processed[feature] = -1 # Default if no encoder
    ...
    try:
        ...
        return None # Return None if scaling fails
    ...
    # Ensure consistent feature order and handle potential NaNs after processing
    df_all_features_reindexed = df_all_features.reindex(columns=all_model_features, fill_value=0)
    df_all_features_filled = df_all_features_reindexed.fillna(0) # Fill any NaNs that might have crept in

    return df_all_features_filled

# --- (preprocess_input and display_car_card functions remain the same) ---
def preprocess_input(input_data, label_encoders, scaler, all_model_features, numerical_features_scaled, numerical_features_unscaled, categorical_features):
    """Preprocesses the SINGLE input data row for prediction."""
    ...
    input_df.replace([np.inf, -np.inf], 0, inplace=True) # Replace inf generated by division
    ...
    return final_input_df.fillna(0) # Fill NaNs in the final input vector

def display_car_card(car_data, card_type="similar"):
    """Displays car data in a styled card format with improved error handling."""
    # Default values
    title = "N/A"
    price_label = "Price"
    price_value = "$0"
    ...
    try:
        # Determine title and price based on card type
        if card_type == "similar":
            ...
            selling_price = car_data.get('sellingprice') # Get value first
            if pd.notna(selling_price) and isinstance(selling_price, (int, float)):
                price_value = f"${selling_price:,.0f}"
            else:
                price_value = "N/A" # Or "$0"

            # Robustly get and format content details
            ...
            # Condition formatting needs care if scale isn't always 5
            ...
            make = car_data.get('Make', 'N/A') # Note different key capitalization
            model = car_data.get('Model', 'N/A')
            title = f"{make} {model}"
            price_label = "Predicted Price"
            price_value = car_data.get('Predicted Price', '$0') # Assumes already formatted string
        ...
        # Construct the card HTML - REMOVED HTML comments /* */
        # Ensure all variables are strings before inserting into HTML
        title_str = str(title)
        ...
        st.exception(e) # Show the actual exception and traceback in the app
        # Optionally display raw data when card fails
        try:
            st.json(car_data, expanded=False) # Show data that caused the error
        except Exception:
...
# --- App Start ---
# Display Banner Image using the user's path
if os.path.exists(BANNER_IMAGE_PATH):
    ...
    # Optional: Add a fallback placeholder
...
# --- Define Feature Sets (Ensure consistency with model training) ---
all_model_features = ['mmr', 'price_per_mile', 'odometer', 'condition', 'mileage_per_year', 'model', 'year', 'vehicle_age', 'make'] # Verify order!
...
if preprocessed_data_for_similarity is None:
    ...
    # Optionally stop the app or disable the feature
    # st.stop()
    ...
    with st.expander("Enter Car Details Here", expanded=True):
        ...
        with col1:
            ...
            if not filtered_models:
                ...
                if filtered_models: # Check if list is not empty
                    try:
                        # Try finding a common model, otherwise default to 0
                        common_models = ['Sedan', 'Base', 'LX', 'EX', 'LE', 'XLE'] # Add more common trims/models
                        for common in common_models:
                           if common in filtered_models:
                                default_model_index = filtered_models.index(common)
                                break
                    except ValueError:
                        default_model_index = 0 # Fallback if common not found
                model_selected = st.selectbox('Select Model', filtered_models, key='pred_model', index=default_model_index)

            # Ensure condition range is valid before creating slider
            ...
            if min_cond >= max_cond: max_cond = min_cond + 4.0 # Ensure range exists
        ...
        with col2:
            ...
            mmr = st.number_input('Estimated Market Value (MMR $)', min_value=0.0, value=median_mmr,
                                  step=100.0, format="%.0f", key='pred_mmr', # Changed format to integer
                                  help="Manheim Market Report value, if known (optional).")

        # --- Input Validation (remains same) ---
        validation_passed = True
        # ... (validation checks) ...
        if year > current_year: st.error("Year cannot be in the future!"); validation_passed = False
        if odometer < 0: st.error("Odometer cannot be negative!"); validation_passed = False
        if mmr < 0: st.error("MMR cannot be negative!"); validation_passed = False
        if make and not model_selected and filtered_models: st.warning("Please select a valid model."); validation_passed = False


    if st.button('Predict Selling Price', key='predict_button', disabled=not validation_passed, use_container_width=True):
        # --- Prediction Logic ---
        if validation_passed and model_selected and preprocessed_data_for_similarity is not None: # Check if preprocessed data exists
            ...
            with st.spinner('Performing Analysis... Please Wait...'):
                ...
                # Preprocess only the single input row
                ...
                if final_input is not None:
                    ...
                    try:
                        ...
                        # ===> ADJUST PRICE SCALING BACK <===
                        pred_price = pred_log_price * 100000 # Example
                        # ====================================
                        ...
                        prediction_done_time = time.time() # Optional timer point

                        # --- Find Similar Cars (using precomputed data) ---
                        ...
                        # Calculate similarity against the preprocessed dataset
                        similarity_scores = cosine_similarity(final_input, preprocessed_data_for_similarity) # Much faster now!

                        # Get top N indices (ensure enough data points exist)
                        num_similar_to_show = 5
                        num_available = len(preprocessed_data_for_similarity)
                        # Get indices from N+1 highest similarity scores, excluding the top one (potential self-match)
                        if num_available > 1 :
                             ...
                             top_indices = [0] # Only one item exists
                        else:
                             top_indices = [] # No data


                        similar_cars_df = df.iloc[top_indices][['make', 'model', 'year', 'condition', 'odometer', 'mmr', 'sellingprice', 'state']].copy()
                        similarity_done_time = time.time() # Optional timer point

                        # --- Display Similar Cars in Cards ---
                        if not similar_cars_df.empty:
                             cols = st.columns(len(similar_cars_df)) # Use as many columns as cars found (up to N)
                        ...
                        display_done_time = time.time() # Optional timer point

                        # Optional: Print timing for debugging
                        # st.caption(f"Debug Timing: Predict={prediction_done_time - start_time:.2f}s, Similarity={similarity_done_time - prediction_done_time:.2f}s, Display={display_done_time - similarity_done_time:.2f}s")

                        # --- Save Prediction Option (remains same) ---
...
# === TAB 2: Price Range Explorer (Remains the same) ===
with tab2:
    st.header("Explore Market by Price")
    st.write("Filter cars within your desired budget.")
    # ... (Slider and plotting logic remains the same) ...
    ...
    if min_price >= max_price: max_price = min_price + 10000 # Ensure range
    ...
    if default_min >= default_max : default_min = min_price; default_max = max_price # Final range check
    ...
    if not filtered_df.empty:
        ...
        col1, col2 = st.columns(2)
        with col1:
            # Ensure 'make' exists and handle potential errors
            ...
            else: st.warning("Column 'make' not found for plotting.")
        with col2:
             ...
             else: st.warning("Columns 'odometer' or 'sellingprice' not found for plotting.")
...
# === TAB 3: Market Insights (Remains mostly the same, added error checks) ===
with tab3:
    ...
    with col1:
        if 'sellingprice' in df.columns:
            ...
            try:
                ...
                fig_price_hist.update_layout(title_x=0.5); st.plotly_chart(fig_price_hist, use_container_width=True)
            except Exception as e: st.warning(f"Plotting Error (Price Dist): {e}")
        else: st.warning("'sellingprice' column needed for Price Distribution.")
        ...
        if 'make' in df.columns and 'sellingprice' in df.columns:
            ...
            try:
                ...
                fig_avg_price.update_traces(texttemplate='$%{text:,.0f}', textposition='outside'); fig_avg_price.update_layout(xaxis={'categoryorder':'total descending'}, title_x=0.5); st.plotly_chart(fig_avg_price, use_container_width=True)
            except Exception as e: st.warning(f"Plotting Error (Avg Price by Make): {e}")
        else: st.warning("'make' and 'sellingprice' columns needed for Avg Price by Make.")
    ...
    with col2:
        if 'odometer' in df.columns and 'sellingprice' in df.columns:
            ...
            try:
                ...
                if sample_size > 0:
                     ...
                     fig_price_mileage.update_layout(title_x=0.5); st.plotly_chart(fig_price_mileage, use_container_width=True)
                else: st.info("Not enough data to plot Price vs Mileage Density.")
            except Exception as e: st.warning(f"Plotting Error (Price vs Mileage): {e}")
        else: st.warning("'odometer' and 'sellingprice' columns needed for Price vs Mileage.")
        ...
        if 'year' in df.columns and 'sellingprice' in df.columns:
             ...
             try:
                 ...
                 fig_price_year.update_layout(title_x=0.5); st.plotly_chart(fig_price_year, use_container_width=True)
             except Exception as e: st.warning(f"Plotting Error (Price Trend by Year): {e}")
        else: st.warning("'year' and 'sellingprice' columns needed for Price Trend.")

    # Geographic Map (Remains same, add check for state data)
    if 'state' in df.columns and 'sellingprice' in df.columns and not df['state'].empty:
        ...
        try:
            ...
            # Optional: Add count for hover data if needed
            # state_counts = df['state'].value_counts().reset_index(); state_counts.columns = ['state', 'count']; state_data = pd.merge(state_avg_price, state_counts, on='state')
            state_data = state_avg_price # Use only avg price for color
            ...
            fig_map.update_layout(title_x=0.5, geo=dict(bgcolor='rgba(0,0,0,0)'), margin={"r":0,"t":40,"l":0,"b":0}); st.plotly_chart(fig_map, use_container_width=True)
        except ImportError: st.warning("⚠️ Plotly Geo library might be needed.")
        except Exception as e: st.warning(f"Could not generate map. Check 'state' format. Error: {e}")
    #else: st.info("Add 'state' and 'sellingprice' columns with data for geographic map.")


# === TAB 4: Saved Predictions (Remains mostly the same) ===
with tab4:
    ...
    if st.session_state.predictions:
        ...
        # Reverse order to show newest first, handle potential errors in data
        for i, pred in enumerate(reversed(st.session_state.predictions)):
            ...
            with cols[col_index]:
                try:
                    ...
                    st.json(pred) # Show raw data if card fails
