...
def load_data(filepath):
    ...
    try:
        # Load data with explicit encoding and delimiter
        # Using chunks to reduce memory usage
        chunk_size = 5000  # Adjust based on your needs
        chunks = pd.read_csv(filepath, encoding='utf-8', sep=',', chunksize=chunk_size)
        df = pd.concat(chunks, ignore_index=True)
...
def get_preprocessed_data_for_similarity(_df_original, _label_encoders, _scaler,
                                        all_model_features, numerical_features_scaled,
                                        numerical_features_unscaled, categorical_features):
    # Sample a smaller subset for similarity search to speed up preprocessing
    sample_size = 1000  # Reduced from 10,000 to 1,000 rows for faster processing
    if len(_df_original) > sample_size:
        df_processed = _df_original.sample(n=sample_size, random_state=42).copy()
    else:
        df_processed = _df_original.copy()

    # Optimize categorical encoding using vectorized operations
    for feature in categorical_features:
        if feature in _label_encoders:
            le = _label_encoders[feature]
            classes = set(le.classes_)
            # Create a mapping dictionary for faster lookup
            class_to_label = {cls: idx for idx, cls in enumerate(le.classes_)}
            # Vectorized mapping: map values to labels, use -1 for unseen values
            df_processed[feature] = df_processed[feature].map(class_to_label).fillna(-1).astype(int)
        else:
            df_processed[feature] = -1
